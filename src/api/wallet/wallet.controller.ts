...
import { CreateWalletDto, ExportWalletDto } from "./wallet.dto";
...
export class WalletController {
    ...
    @OkResponse()
    async exportWallet(req: Request) {
        const context = extractContext(req);
        const jwtPayload = extractRequest<JwtPayload>(req, "user");
        const dto = extractRequest<ExportWalletDto>(req, "body");
        const { id } = extractRequest<{ id: ObjectId }>(req, "params");
        return walletService.exportWallet(context, {
            ...dto,
            walletId: id,
            userId: toObjectId(jwtPayload.userId),
        });
